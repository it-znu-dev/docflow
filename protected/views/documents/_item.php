...
<div class="dfbox" id="<?php echo $model->idDocument; ?>_doc">
  <div class="row dfmetaheader">
    ...
    <div class="col-md-4 col-xs-12 col-sm-6 doc-info-container"><ul><li>
      <?php 
      //якщо користувач має права адмініструання документів
      if(Yii::app()->user->checkAccess('_DocsAdmin')){ ?>
      номер: 
      <?php //if ($model->SubmissionIndex){ ?>
      <?php $controller->echoInfoContainer($model,'SubmissionIndex',$model->idDocument,'text',$model->SubmissionIndex); ?>
       </li>
      <?php //} ?>
      <li>категорія:
      <?php $controller->echoInfoContainer($model,'CategoryID',$model->idDocument,'select',
        $model->_document_doccategory->CategoryName . ' ' .$model->_document_doccategory->CategoryCode); ?>
      </li><li>
      дата надходження:
      <?php $controller->echoInfoContainer($model,'SubmissionDate',$model->idDocument,'text',$model->SubmissionDate); ?>
      </li>
      <?php  } 
        ...
        <?php $controller->echoInfoContainer($model,'SubmissionInfo',$model->idDocument,'text',
    ...
    <div class="col-md-4 col-xs-12 col-sm-6 doc-info-container"><ul>
      <li>
      <?php $controller->echoInfoContainer($model,'ExternalIndex',$model->idDocument,'text',
        $model->ExternalIndex); ?></li><li>тип:
      <?php $controller->echoInfoContainer($model,'TypeID',$model->idDocument,'select',
        $model->_document_doctype->TypeName); ?>
      </li></ul>
    </div>
  ...
  <div class="row row-nomargins">
    ...
    <div class="col-xs-12 col-sm-6 col-md-3">
      <div class="dfblock <?php echo $oncontrol; ?>">
        <legend>Відмітки контролю</legend>
          <?php 
            if(((strlen(trim($model->ControlDate)) > 0) || (strlen(trim($model->ControlMark)) > 0)) || 
    ...
    <div class="col-xs-12 col-sm-6 col-md-3">
      <div class="dfblock <?php echo $done; ?>">
        <legend>Відмітки виконання</legend><ul class='marks'>
          <?php 
  ...
  <div class="row row-nomargins">
    <div class="col-xs-12 col-sm-6 col-md-3">
      <div class="dfblock">
        <legend>Кореспондент</legend>
          <?php $controller->echoInfoContainer($model,'Correspondent',$model->idDocument,'text',
            $model->Correspondent); ?>
      </div>
    </div>
    <div class="col-xs-12 col-sm-6 col-md-3">
    ...
    <div class="col-xs-12 col-sm-6 col-md-3">
      <div class="dfblock">
        <legend>На кого розписано (резолюція)</legend>
    ...
    <div class="col-xs-12 col-sm-6 col-md-3">
      <div class="dfblock">
        ...
        ?>
        <input id="<?php echo $model->idDocument; ?>ytDocuments_file" type="hidden" value="" name="Documents[file]" />
        <input id="<?php echo $model->idDocument; ?>fileupload" 
            type="file" 
            name="Documents[file]" 
            data-url="<?php echo Yii::app()->CreateUrl("documents/update",array('id' => $model->idDocument,
              'Documents[file]'=>'1')); ?>"
            style="display: none;" />
        <a href="#" onclick="$('#<?php echo $model->idDocument; ?>fileupload').click();return false;" title="Додати файл">
          ...
          <div class="scrolls">
            ...
            <?php for($i = 0; $i < count($model->_document_flows); $i++){  ?>
              <div class="flow-block">
                <legend><?php echo CHtml::link(date("d.m.Y H:i",strtotime($model->_document_flows[$i]->Created)),
                  Yii::app()->CreateUrl('flows/index',array('Flows[idFlow]'=>$model->_document_flows[$i]->idFlow)),
                  array('target'=>"_blank")); ?> - 
                  <?php echo "<u title='"
              ...
              <?php foreach($model->_document_flows[$i]->_flow_flow_respondent as $flow_resp){ ?>
                <div class="resp resp-<?php echo ($flow_resp->AnswerID)? "green":"brown"; ?>"
                  ...
                  title="<?php echo ($flow_resp->AnswerID)? 
                    "Надано користувачем: ".$flow_resp->_flow_respondent_answer->_answer_user->info." ".date("d.m.Y H:i",strtotime($flow_resp->_flow_respondent_answer->Created))
                      .((strlen(trim($flow_resp->_flow_respondent_answer->AnswerText)) > 0)? " (коментар: ".trim($flow_resp->_flow_respondent_answer->AnswerText).")" : "")
                    :""; ?>">
                  <span class="glyphicon glyphicon-<?php echo ($flow_resp->AnswerID)? "ok":"ban"; ?>-circle"></span>
