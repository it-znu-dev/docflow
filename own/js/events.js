  ...
  function MultiCalendar(
    ...
    this.appendMonthCalendarTable = function(p_month,p_year){
      ...
      for (var i = 0; i < calendar_array.length; i++){
        ...
        for (var j = 0; j < 7; j++){
          ...
          if ((i===0 && calendar_array[i][j] < 10) 
            || (i===(calendar_array.length-1) && calendar_array[i][j] > 10) 
            || (i > 0 && i < calendar_array.length-1)){
            $("#"+table_id+" tbody tr."+tr_class).append(
                    '<td id="'+td_id+'">'+calendar_array[i][j]+'</td>');
          } else {
            $("#"+table_id+" tbody tr."+tr_class).append(
                    '<td class="passive">'+calendar_array[i][j]+'</td>');
          }
        }
      }
    };
    ...
    this.putCalendar = function(){
      ...
      if (_self.calendar_url){
        _self.$calendar_block.append(
          "<a href='#' class='glyphicon glyphicon-backward' id='"+_self.calendar_block_id+"_prev'></a>"
        );
      ...
      if (_self.calendar_url){
        $.ajax({
          ...
          success: function(data){
            ...
            var append_template = '<a href="'
                    +_self.calendar_url
                    +'/../admin?Events[date_search]={EventDate}" '
        ...
        _self.$calendar_block.append(
          "<a href='#' class='glyphicon glyphicon-forward' id='"+_self.calendar_block_id+"_next'></a>"
        );
    ...
    this.parseRule = function(rule_string){
      ...
      for (var  i = 0; i < _self.patterns.length; i++){
        ...
        if (mtchs !== null){
          switch (i){
            case 0:
              ret.start_date = new Date(parseInt(mtchs[3],10),
                parseInt(mtchs[2],10)-1,parseInt(mtchs[1],10));
              ret.end_date = new Date(parseInt(mtchs[6],10),
                parseInt(mtchs[5],10)-1,parseInt(mtchs[4],10));
              if (
              ...
              break;
            case 1:
              ret.start_date = new Date(parseInt(mtchs[4],10),
                parseInt(mtchs[3],10)-1,parseInt(mtchs[1],10));
              ret.end_date = new Date(parseInt(mtchs[4],10),
                parseInt(mtchs[3],10)-1,parseInt(mtchs[2],10));
              if (
                  ret.start_date == 'Invalid Date'
                  || ret.end_date == 'Invalid Date'
                 ){
            ...
            case 2:
              ...
              break;
            case 3:
              ret.start_date = new Date(_self._start_date.getFullYear(),
                parseInt(mtchs[2],10)-1,parseInt(mtchs[1],10));
              ret.end_date = new Date((_self._start_date.getFullYear() + _self.m_years),
                parseInt(mtchs[2],10)-1,parseInt(mtchs[1],10));
              ret.step = [1,0,0];
              if (
                  ret.start_date == 'Invalid Date'
            ...
            case 5:
              ...
              if (mtchs[2] === undefined){
                ...
                ret.start_date = new Date(parseInt(mtchs[5],10),
                  parseInt(mtchs[4],10)-1,parseInt(mtchs[3],10));
              }
              if (
                  ret.start_date == 'Invalid Date'
                  || ret.end_date == 'Invalid Date'
              ...
              if (mtchs[2] === undefined){
                ret.end_date = new Date(ret.start_date.getFullYear(),
                  ret.start_date.getMonth()+4,0);
              } else {
                ret.end_date = new Date(parseInt(mtchs[8],10),
                  parseInt(mtchs[7],10)-1,parseInt(mtchs[6],10));
              }
              ret.step = [0,0,7];
              return ret;
              break;
    ...
    this.getDatesByRules = function(rules_str){
      ...
      for (var i = 0; i < a_rules.length && k < 1000; i++ ){
        for (var dt = a_rules[i].start_date; 
            dt <= a_rules[i].end_date  && k < 1000; k++ ){
    ...
    } //end if (_self.input_field_id !== null)
